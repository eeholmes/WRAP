...
If a new vignette is added to vignettes folder run this. `build_vignettes` is putting the `doc` folder at the base level instead of keeping in `inst` because normally you make the vignette files when building but I don't want to do that since they take a long time. Note if pandoc won't build vignettes, try closing everything and restarting computer.
...
Then make sure `doc` wasn't added to bottom of .gitignore. Then rebuild.  After that this will show the vignettes:
