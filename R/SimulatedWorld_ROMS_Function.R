...
SimulateWorld_ROMS <- function(
  ...
  for (y in august_indexes){
    ...
    if("sst" %in% covariates) sst <- raster(files_sst[y])
  ...
  if (abund_enviro == "lnorm_low") {
    ...
    output$abundance <- ifelse(output$pres==1,rlnorm(n.samples*n.year,2,0.1)*output$suitability,0)
  }
  if (abund_enviro == "lnorm_high") {
    ...
    output$abundance <- ifelse(output$pres==1,rlnorm(n.samples*n.year,6,1)*output$suitability,0)
  }
  if (abund_enviro == "poisson") {
    ...
    output$abundance <- ifelse(output$pres==1,rpois(n.samples*n.year, lambda=output$suitability*maxN),0)
  } 
